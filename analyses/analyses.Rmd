...
data_generic_pattern <- 
  data.frame(trial_type = c("Category 1\npositive",
                            ...
                            "Category 2\npositive",
                            "Category 2\nnegative"),
             D_mean     = c(+0.342, +0.138, -0.199, +0.019), # values calculated manually from lmer results table, ie intercept + level
             D_ci_lower = c(+0.300, +0.062, -0.275, -0.058),
             D_ci_upper = c(+0.383, +0.214, -0.123, +0.094),
...
data_emms <- summary(emmeans(fit, 1 ~ trial_type:Domain))

plot_domains <- 
  ggplot() +
  geom_hline(yintercept = c(0), linetype = "dotted") +
  geom_linerange(data = data_emms, 
                 aes(x = trial_type, 
                     ymax = lower.CL,
                     ymin = upper.CL,
                     colour = Domain),
  ...
  geom_point(
    data = data_emms, aes(x = trial_type, y = emmean, colour = Domain),
    #aes(shape = Domain),
    size = 1.5,
    position = position_dodge(width = .4)
  ) +
...
#### Bootstrap effect size estimates

via case removal and percentile method with 2000 replications.
...
data_geta_wrangled <- boot_estimates_domain |>
  filter(Effect != "(Intercept)") |>
  select(Effect, getasq_median, getasq_se) |>
  pivot_wider(names_from = "Effect", 
              values_from = c("getasq_median", "getasq_se")) |>
  janitor::clean_names() |>
  select(trialtype_est = getasq_median_trial_type,
         trialtype_se = getasq_se_trial_type,
         domain_est = getasq_median_domain,
         domain_se = getasq_se_domain,
         interaction_est = getasq_median_domain_trial_type,
...
plot_stimuli <- 
  ...
  geom_line(
    data = data_emms_2, aes(x = trial_type, y = emmean, colour = Domain, group = Domain),
    position = position_dodge(width = .2)
  ) +
  geom_point(
    data = data_emms_2, aes(x = trial_type, y = emmean, colour = Domain, shape = Domain),
    size = 2.5,
    position = position_dodge(width = .2)
  ) +
  ylab(expression(paste(italic("D"), " score"))) +
...
data_long_unknown_stimuli |>
  ...
  ungroup() |>
  select(-category_1_positive, -category_2_positive, -category_1_negative, -category_2_negative) |>
